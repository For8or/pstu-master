<div>
    ...
    <x-jet-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('Створити API Токен') }}
        </x-slot>

        <x-slot name="description">
            {{ __('API токени дозволяють службам третіх сторін автентифікуватися за допомогою нашої програми від вашого імені.') }}
        </x-slot>
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('Керування API Токенами') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Ви можете видалити будь-які наявні маркери, якщо вони більше не потрібні.') }}
                </x-slot>
                                ...
                                <div class="flex items-center">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ __('Останнє використання') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
