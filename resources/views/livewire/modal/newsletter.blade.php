<div>
    ...
    <x-jet-dialog-modal wire:model="confirmingSubscription">
        <x-slot name="title">
            {{ __('Підпишіться на нашу розсилку') }}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <form wire:submit.prevent="submit">
                ...
                {{-- Name --}}
                <input wire:model="name" type="text" placeholder="Імʼя" class="block w-full mt-5 rounded-md shadow-sm border-theme-olive focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                <x-jet-input-error for='name' class="my-2 font-medium" />

                {{-- Email --}}
                <input wire:model="email" type="email" placeholder="Пошта" class="block w-full mt-5 rounded-md shadow-sm border-theme-olive focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
